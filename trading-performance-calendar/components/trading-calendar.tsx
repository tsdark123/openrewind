...
export function TradingCalendar() {
  ...
  return (
    <section
      ...
      className="w-full max-w-[420px] rounded-[22px] border border-border bg-panel p-3 shadow-2xl shadow-black/40"
    >
      {/* metric selector */}
      <div className="px-1 pb-3 pt-1">
        <MetricSelect label={metricLabel} value={metric} onChange={setMetric} />
      </div>
          ...
          <div className="grid grid-cols-7 gap-px overflow-hidden rounded-[6px] bg-grid-line">
            {cells.map(({ date, outside }) => (
              <DayCell
                ...
                outside={outside}
                metric={metric}
                stats={outside ? null : getDayStats(date.getFullYear(), date.getMonth(), date.getDate())}
              />
            ))}
          </div>
        </>
...
function NavButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      aria-label={label}
      className="flex size-7 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-popover hover:text-foreground"
    >
      {children}
    </button>
  )
  ...
  return (
    <div
      ...
      className={cn(
        ...
        !outside && tone === 'gold' && 'bg-neutralish',
...
type Tone = 'none' | 'flat' | 'gold' | 'profit' | 'profit-strong' | 'loss' | 'loss-strong'

function dayTone(profit: number | undefined): Tone {
  if (profit === undefined) return 'none'
  const abs = Math.abs(profit)
  if (abs < 100) return 'flat' // scratch session, no tint
  if (profit > 0) return abs < 500 ? 'gold' : abs < 2000 ? 'profit-strong' : 'profit'
  return abs < 2000 ? 'loss' : 'loss-strong'
}
        ...
        return (
          <button
            ...
            className={cn(
              ...
              level === 0 && 'bg-cell',
              level > 0 && positive && (level === 2 ? 'bg-profit' : 'bg-profit-strong'),
              level > 0 && !positive && (level === 2 ? 'bg-loss-strong' : 'bg-loss'),
            )}
          >
            <span
              className={cn('text-[11px]', level === 0 ? 'text-muted-foreground' : 'text-tint-fg/80')}
            >
              {name.slice(0, 3)}
            </span>
            <span
  ...
  return (
    <div ref={ref} className="relative inline-block">
      <button
        ...
        aria-expanded={open}
        className="flex items-center gap-1.5 rounded-md border border-border bg-cell px-2.5 py-1.5 text-[12px] text-foreground/90 transition-colors hover:bg-popover"
      >
        {label}
      ...
      {open && (
        <ul
          role="listbox"
          className="absolute left-0 top-[calc(100%+4px)] z-10 w-40 overflow-hidden rounded-lg border border-border bg-popover py-1 shadow-xl shadow-black/50"
        >
          {METRICS.map((m) => (
            <li key={m.id}>
              <button
