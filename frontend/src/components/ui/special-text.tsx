...
export function SpecialText({
  ...
  useEffect(() => {
    if (displayText === text && hasStarted && onComplete) {
      const id = setTimeout(onComplete, speed);
      return () => clearTimeout(id);
    }
  }, [displayText, text, hasStarted, speed, onComplete]);

  return (
    <span
      ref={containerRef}
      className={`h-5 leading-5 inline-flex font-mono font-medium ${className}`}
    >
      {displayText}
    </span>
  );
