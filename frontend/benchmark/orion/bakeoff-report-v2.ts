...
export function formatV2Scorecard(scorecard: V2ModelScorecard): string {
  ...
  const lines = [
    ...
    `| Critical context prompt pass rate | ${pct(scorecard.criticalContextPromptPassRate)} |`,
    '',
    `| Hardcoding audit | ${scorecard.hardcodingAuditPassed ? 'pass' : 'fail'} |`,
    `| Context regression | ${scorecard.contextRegressionPassed ? 'pass' : 'fail'} |`,
    `| Analysis acceptance | ${scorecard.analysisAcceptancePassed ? 'pass' : 'fail'} |`,
    `| Runtime acceptance | ${scorecard.runtimeAcceptancePassed ? 'pass' : 'fail'} |`,
    '',
